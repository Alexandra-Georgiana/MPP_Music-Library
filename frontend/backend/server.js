...
console.log('Static path:', staticPath); // Debug log
...
const callFlaskApi = async (endpoint, data, fallbackEndpoint = null) => {
  ...
  try {    // Try the primary endpoint
    let response = await fetch(`${FLASK_API_URL}${endpoint}`, {
      method: 'POST',
    ...
    if (response.status === 404 && fallbackEndpoint) {
      ...
      response = await fetch(`${FLASK_API_URL}${fallbackEndpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      });
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    const checkRes = await fetch(`${FLASK_API_URL}/getUserByEmail`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
app.post('/api/addSong', upload.fields([
  ...
  try {
    ...
    const audioUrl = `${NODE_API_URL}/uploads/${files.audioFile[0].filename}`;
    const albumImage = `${NODE_API_URL}/uploads/${files.albumCover[0].filename}`;
