...
try {
    ...
    if (-not (Test-Path -Path "frontend/backend/uploads")) {
        ...
        Write-ColorMessage "✓ Created uploads directory" "Green"
    }
    else {
        Write-ColorMessage "✓ Uploads directory already exists" "Green"
    ...
    if (Test-Path -Path $repoPath -PathType Container) {
        ...
        Write-ColorMessage "⚠ $repoPath is a directory! Renaming to $newName" "Yellow"
    ...
    if (Test-Path -Path $serverPath -PathType Container) {
        ...
        Write-ColorMessage "⚠ $serverPath is a directory! Renaming to $newName" "Yellow"
    ...
    if (Test-Path -Path "app" -PathType Container) {
        ...
        Write-ColorMessage "⚠ Found 'app' directory which conflicts with Railway deployment!" "Red"
        Write-ColorMessage "Renaming 'app' to '$newName'..." "Yellow"
        Rename-Item -Path "app" -NewName $newName -Force
        Write-ColorMessage "✓ Resolved 'app' directory conflict" "Green"
    }    # Check for node_modules/app directories that might cause conflicts
    Write-ColorMessage "Creating comprehensive .railwayignore file..." "Yellow"
    
    # Create a more robust .railwayignore file
    ...
    if (Test-Path -Path $nixpacksFile) {
        ...
        if ($nixpacksContent -match "\[phases\.build\]") {
            if ($nixpacksContent -notmatch "output\s*=") {
                $newNixpacksContent = $nixpacksContent -replace "(\[phases\.build\][^\[]*)", "`$1`noutput = ""railway-app""`n"                $newNixpacksContent | Out-File -FilePath $nixpacksFile -Force
...
"@
        ...
        Write-ColorMessage "⚠️ deploy-nixpacks.toml not found, creating minimal configuration..." "Yellow"
...
"@
        ...
        Write-ColorMessage "✓ Created minimal $nixpacksFile with custom output name" "Green"
    }
    ...
    if (Test-Path -Path "railway-up.js") {
        ...
        Write-ColorMessage "✓ Created backup: railway-up.js.bak" "Green"
...
'@
                ...
                $newContent | Out-File -Path "railway-up.js" -Encoding utf8 -Force -NoNewline
                Write-ColorMessage "✓ Enhanced directory validation in railway-up.js" "Green"
            } else {
                Write-ColorMessage "⚠️ Could not find the right section to enhance in railway-up.js" "Yellow"
            }
        } else {
            Write-ColorMessage "✓ railway-up.js already has robust directory validation" "Green"
        }
    } else {
        Write-ColorMessage "⚠️ railway-up.js not found, skipping enhancement" "Yellow"
    }
    ...
    if (Test-Path -Path "pre-deploy.sh") {
        ...
        Write-ColorMessage "✓ Created backup: pre-deploy.sh.bak" "Green"
...
'@
            ...
            if ($preDeployContent -match "^#!/bin/bash") {
                ...
                Write-ColorMessage "✓ Enhanced pre-deploy.sh with directory conflict handling" "Green"
            } else {
                # Just add to the beginning if no shebang found
                $newPreDeployContent = "#!/bin/bash$conflictHandler`n$preDeployContent"
                $newPreDeployContent | Out-File -Path "pre-deploy.sh" -Encoding utf8 -Force -NoNewline
                Write-ColorMessage "✓ Added directory conflict handling to pre-deploy.sh" "Green"
            }
        } else {
            Write-ColorMessage "✓ pre-deploy.sh already has directory conflict handling" "Green"
        }
    }
